...
const CONTROL_PLANE_NAME: &str = "rust-k8s-control-plane";
const WORKER_PREFIX: &str = "rust-k8s-worker";
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("{}", "🚀 Starting Kubernetes cluster installation with Multipass".green().bold());
    ...
    save_kubeconfig(&config.control_plane.name)?;
    ...
    println!("\n{}", "✅ Cluster installation complete!".green().bold());
...
fn save_kubeconfig(control_plane_name: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if kubeconfig_path.exists() {
        ...
        match fs::rename(&kubeconfig_path, &backup_path) {
            Ok(_) => println!("  📦 Existing kubeconfig backed up to: {}", backup_path.display()),
    ...
    let output = Command::new("multipass")
        .args(&["exec", control_plane_name, "--", "sudo", "cat", "/etc/kubernetes/admin.conf"])
        .output()?;
    ...
    // Get control plane IP
    let control_plane_ip = get_node_ip(control_plane_name)?;
...
fn display_cluster_info(config: &ClusterConfig, control_plane_ip: &str, grafana_password: &str) {
    println!("\n{}", "=".repeat(60));
    println!("{}", "🎉 Kubernetes Cluster Ready!".green().bold());
    println!("{}", "=".repeat(60));
    ...
    println!("  • Stop cluster:         multipass stop --all");
    println!("  • Start cluster:        multipass start --all");
    println!("  • Delete cluster:       multipass delete --all --purge");

    println!("\n🔧 Testing the cluster:");
    println!("  kubectl create deployment nginx --image=nginx");
    println!("  kubectl expose deployment nginx --port=80 --type=NodePort");
    println!("  kubectl get svc nginx");
